...
contains
  ...
  subroutine initialize(self,configunit)
    ...
    call self%register_dependency(&
         self%id_Kc0,'Kc0','-','Henry''s constant',required=.false.)
    call self%register_dependency(&
         self%id_Kc1,'Kc1','-','[H+][HCO3-]/[H2CO3]',required=.false.)
    call self%register_dependency(&
         self%id_Kc2,'Kc2','-','[H+][CO3--]/[HCO3-]',required=.false.)
  ...
  subroutine do_surface(self,_ARGUMENTS_DO_SURFACE_)
    ...
    if (_AVAILABLE_(self%id_Kc0)) then
  ...
  subroutine do(self,_ARGUMENTS_DO_)
    ...
    if (_AVAILABLE_(self%id_Kc1)) then
      _GET_(self%id_Kc1,Kc1)
    else
       Kc1=2.5e-4 
    end if
    if (_AVAILABLE_(self%id_Kc2)) then
