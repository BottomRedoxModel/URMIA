...
!     Variable identifiers
   ...
   real(rk), parameter :: NaCl_density =  3.72E7 ! [mmolNaCl/m3] from = 2.16e6 g/m3
...
!BOC
   ...
   Om_NaCl= Na & !*(1.0_rk-max(0.0_rk,(Na-3.5e6)*0.1e-6)) &
           *Cl & !*(1.0_rk-max(0.0_rk,(Cl-3.5e6)*0.1e-6)) &
           /(self%K_NaCl) & 
   ...
   if (NaCl.gt.NaCl_density)  NaCl_prec=0.0_rk ! no precipitation if cristalls occupy more than 100% of volume
   NaCl_diss=self%K_NaCl_diss*max(0._rk,(1._rk-Om_NaCl))*max(0._rk,NaCl)
   ...
   _SET_ODE_(self%id_Het,-(0.5_rk+0.5_rk*tanh(Na-8.0e6))*0.6_rk*Het)
!   _SET_ODE_(self%id_Het,-(0.5_rk+0.5_rk*tanh(Om_NaCl-1.05_rk))*0.6_rk*Het)
